...
MINUTI=/opt/consumi/consumi-minuto
ORE=/opt/consumi/consumi-ore
TOTALE=/opt/consumi/consumo-totale

sleep 15

cat $MINUTI | awk '{ sum+=$1} END {print sum/NR}' |cut -d. -f1 >> $ORE

rm -f $MINUTI

#### NO CONSUMA TROPPA CPU NO NO NO  ####
# Risommiamo tutte le ore per calcolare il totale che facciamo prima
#cat $ORE | awk '{ sum+=$1} END {print sum}' > $TOTALE

# Adesso aggiorniamo il file dei consumi totali, che non fa altro che sommare
# il vecchio totale con il consumo di quest'ora...

CONSUMO_ULTIMORA=`tail -n1 $ORE`
